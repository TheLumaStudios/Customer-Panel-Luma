-- Customer Panel Database Schema - FIXED VERSION
...
-- =====================================================
-- Uncomment these if you want to recreate everything
...
DROP POLICY IF EXISTS "View own domains" ON domains;
DROP POLICY IF EXISTS "Admins manage domains" ON domains;
DROP POLICY IF EXISTS "View own hosting" ON hosting_packages;
DROP POLICY IF EXISTS "Admins manage hosting" ON hosting_packages;
DROP POLICY IF EXISTS "View own invoices" ON invoices;
DROP POLICY IF EXISTS "Admins manage invoices" ON invoices;
DROP POLICY IF EXISTS "View invoice items" ON invoice_items;
DROP POLICY IF EXISTS "Admins manage invoice items" ON invoice_items;
DROP POLICY IF EXISTS "View own tickets" ON support_tickets;
DROP POLICY IF EXISTS "Customers create tickets" ON support_tickets;
DROP POLICY IF EXISTS "Admins manage tickets" ON support_tickets;
DROP POLICY IF EXISTS "View ticket replies" ON ticket_replies;
DROP POLICY IF EXISTS "Reply to tickets" ON ticket_replies;
...
DROP POLICY IF EXISTS "View own preferences" ON notification_preferences;
...
DROP POLICY IF EXISTS "Admins insert activity logs" ON activity_logs;
DROP POLICY IF EXISTS "Admins view sms logs" ON sms_logs;
DROP POLICY IF EXISTS "Admins insert sms logs" ON sms_logs;
...
CREATE POLICY "Users can view own customer record" ON customers
  FOR SELECT
  USING (profile_id = auth.uid() OR role = 'admin' FROM profiles WHERE id = auth.uid());

CREATE POLICY "Service role can manage customers" ON customers
  FOR ALL
  USING (true);

-- For other tables, we'll use simpler policies that check role via a function
...
-- DOMAINS POLICIES (using function)
