// Supabase Edge Function for enabling/disabling WHOIS privacy
// Deploy: supabase functions deploy domain-whois-privacy
...
serve(async (req) => {
  ...
  try {
    ...
    const { domain_name, enabled } = await req.json()
    ...
    const command = enabled ? 'enable_whois_privacy' : 'disable_whois_privacy'
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `WHOIS privacy ${enabled ? 'enabled' : 'disabled'} successfully`,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  } catch (error) {
    console.error('WHOIS privacy error:', error)
    return new Response(
