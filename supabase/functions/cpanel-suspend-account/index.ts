// Supabase Edge Function for suspending cPanel accounts
// Deploy: supabase functions deploy cpanel-suspend-account
...
serve(async (req) => {
  ...
  try {
    ...
    if (profile?.role !== 'admin') {
      throw new Error('Only admins can suspend cPanel accounts')
    }

    const { hosting_id, reason = 'Payment overdue' } = await req.json()
    ...
    console.log('⏸️ Suspending cPanel account:', hosting.cpanel_username)
    ...
    if (result.metadata?.result !== 1) {
      throw new Error(result.metadata?.reason || 'Failed to suspend cPanel account')
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'cPanel account suspended successfully',
      }),
      {
    ...
    console.error('cPanel suspend error:', error)
    return new Response(
